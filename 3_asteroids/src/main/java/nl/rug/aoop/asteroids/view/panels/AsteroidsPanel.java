...
public class AsteroidsPanel extends JPanel implements GameUpdateListener {
    ...
    /**
     * The x and y coordinates of the player names position
     */
    private static final Point SCORE_PLAYERS_POSITION = new Point(20, 60);
    /**
    ...
    private void drawShipInformation(Graphics2D graphics2D) {
        ...
        for (int i = 0; i < game.getScoreMap().size(); i++) {
            drawStrings(graphics2D, i);
        }
        graphics2D.setColor(Color.GREEN);
        graphics2D.drawRect(SCORE_INDICATOR_POSITION.x, SCORE_INDICATOR_POSITION.y,
                100, ENERGY_BAR_HEIGHT);
        graphics2D.fillRect(SCORE_INDICATOR_POSITION.x, SCORE_INDICATOR_POSITION.y,
                (int) game.getSpaceShip().getEnergyPercentage(), ENERGY_BAR_HEIGHT);
    }

    private void drawStrings(Graphics2D graphics2D, int i) {
        graphics2D.drawString(
                String.valueOf(game.getScoreMap().get(i).getB()),
                SCORE_PLAYERS_POSITION.x,
                SCORE_PLAYERS_POSITION.y + (17 * i)
        );
        graphics2D.drawString(
                String.valueOf(game.getScoreMap().get(i).getA()),
                SCORE_PLAYERS_POSITION.x + 20,
                SCORE_PLAYERS_POSITION.y + (17 * i)
        );
    }
    ...
    private void drawGameObjects(Graphics2D graphics2D) {
        ...
        synchronized (game) {
             ...
             */
            game.getSpaceshipCollection().forEach(spaceship -> new SpaceshipViewModel(spaceship).drawObject(graphics2D, timeSinceLastTick));
